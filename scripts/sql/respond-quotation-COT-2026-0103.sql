-- =============================================================================
-- COT-2026-0103 — Bootstrap rodada 1 + respostas automáticas (teste manual)
-- Fornecedores: TechVision Sistemas, Metalúrgica Souza Ltda, FastLog Transportes
-- NÃO é migration. Rodar no Supabase SQL Editor (postgres).
-- =============================================================================

-- DIAGNÓSTICO (rode primeiro — se rodadas = 0, confira se a cotação foi enviada)
SELECT
...
WHERE (
    qs.supplier_name ILIKE '%TechVision%'
    OR qs.supplier_name ILIKE '%Metalúrgica Souza%'
    OR qs.supplier_name ILIKE '%Metalurgica Souza%'
    OR qs.supplier_name ILIKE '%FastLog%'
  )
  AND NOT EXISTS (
...
-- B) Respostas com preços (3 fornecedores — TechVision mais competitivo)
WITH ctx AS (
  SELECT q.id AS quotation_id, q.company_id, r.id AS round_id, r.round_number
  FROM public.quotations q
  JOIN public.quotation_rounds r ON r.quotation_id = q.id
  WHERE q.code = 'COT-2026-0103' AND r.status = 'active'
  ORDER BY r.round_number DESC
  LIMIT 1
),
pay AS (
...
suppliers AS (
  SELECT qs.*,
    CASE
      WHEN qs.supplier_name ILIKE '%TechVision%' THEN 1
      WHEN qs.supplier_name ILIKE '%Metalúrgica Souza%'
        OR qs.supplier_name ILIKE '%Metalurgica Souza%' THEN 2
      WHEN qs.supplier_name ILIKE '%FastLog%' THEN 3
      ELSE 99
    END AS slot
  FROM public.quotation_suppliers qs
  JOIN ctx ON ctx.quotation_id = qs.quotation_id
  WHERE qs.supplier_name ILIKE '%TechVision%'
     OR qs.supplier_name ILIKE '%Metalúrgica Souza%'
     OR qs.supplier_name ILIKE '%Metalurgica Souza%'
     OR qs.supplier_name ILIKE '%FastLog%'
),
...
priced AS (
  SELECT s.supplier_id, s.supplier_name, s.supplier_cnpj, s.company_id, s.slot,
    ...
    ROUND(
      i.ref_price
      * CASE s.slot
          WHEN 1 THEN 0.94   -- TechVision: melhor preço
          WHEN 2 THEN 1.02   -- Metalúrgica Souza: intermediário
          WHEN 3 THEN 1.10   -- FastLog: mais caro
          ELSE 1
        END
      * (1 - GREATEST(0, (SELECT round_number FROM ctx) - 1) * 0.015),
      2
    ) AS unit_price,
    CASE s.slot
      WHEN 1 THEN 8
      ...
      WHEN 3 THEN 6
      ELSE 9
    END + (i.rn % 3) AS delivery_days
  FROM suppliers s
  CROSS JOIN items i
  WHERE s.slot BETWEEN 1 AND 3
),
upsert_proposals AS (
  INSERT INTO public.quotation_proposals (
  ...
  SELECT ctx.quotation_id, ctx.company_id, ctx.round_id, p.supplier_id, p.supplier_name,
    ...
    MAX(p.delivery_days),
    CASE p.slot
      WHEN 1 THEN 'Proposta automática (teste) — TechVision Sistemas'
      WHEN 2 THEN 'Proposta automática (teste) — Metalúrgica Souza Ltda'
      WHEN 3 THEN 'Proposta automática (teste) — FastLog Transportes'
      ELSE 'Proposta automática (teste)'
    END,
