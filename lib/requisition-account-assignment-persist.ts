...
export async function saveRequisitionItemAccountConfig(
  ...
  if (!config.category) {
    ...
    const { error: deleteError } = await supabase
      .from("requisition_item_account_assignments")
      .delete()
      .eq("requisition_item_id", itemId)
      .eq("company_id", companyId)
...
export async function saveRequisitionAccountConfigs(
  supabase: SupabaseClient,
  companyId: string,
  configs: Record<string, ItemAccountConfigEdit>,
): Promise<{ ok: true } | { ok: false; message: string }> {
  for (const [itemId, config] of Object.entries(configs)) {
    const result = await saveRequisitionItemAccountConfig(
      supabase,
      companyId,
      itemId,
      config,
