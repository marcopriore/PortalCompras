  ...
  return runWithApiKey(
    ...
    async ({ ctx }) => {
      ...
      const includePrevious =
        searchParams.get("include_previous") === "1" ||
        searchParams.get("include_previous") === "true"

      const service = createServiceRoleClient()
      const result = await buildSpendReport(service, ctx.companyId, range, {
        category,
        supplierCode,
        includePrevious,
      })
      ...
      if (!result.ok) {
        ...
        return apiError("Erro ao gerar relatório de spend.", "INTERNAL_ERROR", 500)
      }
